...
class TestConfig(unittest.TestCase):
    ...
    def test_config_to_dict(self):
        for mode in LaunchMode:
            config = Config(mode, self.test_config.starting_path)
            expected = {
                "launchMode": mode.value,
                "startingPath": str(self.test_config.starting_path)
            }
    ...
    def test_config_from_dict_with_valid_data(self):
        for mode in LaunchMode:
            ...
            self.assertEqual(config.starting_path, self.test_config.starting_path)
    ...
    def test_config_from_dict_with_defaults(self):
        ...
        data = {"launchMode": self.test_config.mode.value}
        config = Config.from_dict(data)
        self.assertEqual(config.mode, self.test_config.mode)
        self.assertEqual(config.starting_path, DEFAULT_STARTING_PATH)

        data = {"startingPath": str(self.test_config.starting_path)}
        config = Config.from_dict(data)
        self.assertEqual(config.mode, DEFAULT_LAUNCH_MODE)
        self.assertEqual(config.starting_path, self.test_config.starting_path)
